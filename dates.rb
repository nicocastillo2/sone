...
Campaign.find(3).contacts.each do |c|
  a = c.answer
  month = months.sample
  if month == 1 #or month == 8 or month == 10 or month=12
	day = day_31.sample
  elsif month == 2 #or month == 11
    day = day_28.sample
  else
    day = day_28.sample
  end
  a.created_at = Date.new(2018, month, day)
  a.updated_at = Date.new(2018, month, day)
  a.save
end